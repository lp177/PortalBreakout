...
- `audio.__debug()` reports `{track, style, intensity, intensityTarget, songs, returnTo, baseBpm, bpmNow}` for tests; `window.__pb` now exposes `audio`.

## Music sync, arena-clear bonus, direct map change (v1.3.2)

- **Tempo now tracks intensity, both ways.** v1.3.1 deliberately froze tempo and mapped intensity to brightness/density only, but different songs have different BPM, so an event switch to a faster song left the music sounding permanently quicker than the ball. `runScheduler` now rescales `s.stepDur` from `s.baseStepDur` every tick (`TEMPO_RANGE = 0.2`, so up to +20% and back) and drives the lowpass with `setTargetAtTime` on the same tick. Both previously updated only every four bars, which is what left the track stuck at its peak. Measured: 92 → 110 bpm at full intensity and back to 92 as it decays.
- **Ball count feeds intensity.** `reportIntensity` adds `0.55 * clamp((liveBalls - 1) / 3, 0, 1)` to the speed term: a crowded field is as demanding as a fast one, so multiball is urgent immediately. Measured at a fixed 420 px/s: 1 ball → 118 bpm, 3 → 127, 5 → 131, back to 118.
